...
unsigned long parallel_for_sweep(struct node *head){
  ...
  curr = head;
  struct node** linkedListAsArray;
  linkedListAsArray = (struct node**) malloc(nbCol*sizeof(struct node));  

  int i = 0;
  while(curr){
    linkedListAsArray[i] = curr;
    i++;
    curr = curr->next;
  }


  #pragma omp parallel for reduction (+:acc) num_threads (4)
  for (i=0;i<nbCol;i++) {
      acc+= process_node(linkedListAsArray[i]);
  }

...
ruleFunction par body env = 
    let newVar = newVariable () in
      FunctionType (newVar, type_of_expr body ((par, newVar) ::env))
...
ruleCall fct par env = 
    ...
    match typeFct with
    | FunctionType (tPar, tRes) -> let (_,isOk) = unify (type_of_expr par env) tPar in 
                                      if isOk then tRes else ErrorType
    | _ -> ErrorType 
...
ruleRead expr env = 
    let typeExpr = type_of_expr expr env in 
        ...
        match typeExpr with 
        | ReferenceType(typePointe) -> typePointe
        | _ -> ErrorType
...
ruleWrite left right env = 
    ...
    | ReferenceType(typePointe) -> let (_, isOk) = unify typePointe (type_of_expr right env) in
                                                      if isOk then UnitType else ErrorType
    | _ -> ErrorType

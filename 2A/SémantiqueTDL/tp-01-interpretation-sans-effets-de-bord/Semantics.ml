...
ruleLet env ident bvalue bin = 
  ...
  | value -> value_of_expr bin ((ident,value) :: env)
...
ruleIf env cond bthen belse = 
  match (value_of_expr cond env) with
    | BooleanValue (boolean) -> value_of_expr (if boolean then
                                                   bthen
                                               else 
                                                   belse) env
    | (ErrorValue _) as result -> result 
...
ruleCallByName env fexpr pexpr = 
  ...
  | FrozenValue(FunctionNode(fpar,fbody), envDef) -> value_of_expr fbody ((fpar, FrozenValue(pexpr,envDef)) :: env)
  | (ErrorValue _) as result -> result
  | _ -> ErrorValue TypeMismatchError
(* ========================================================*)
and
...
ruleCallByValue env fexpr pexpr = 
  ...
  | (ErrorValue _) as result -> result
  | value -> value_of_expr fexpr ((,value) :: env)

...
let va = Dag.createv ("a",1,2.);;
let vb = Dag.createv ("b",1,2.);;
let vc = Dag.createv ("c",1,2.);;
let vd = Dag.createv ("d",1,1.);;
let ve = Dag.createv ("e",1,1.);;
let vf = Dag.createv ("f",2,3.);;


Dag.add_edge dag5 va vd;;
Dag.add_edge dag5 vb vd;;
Dag.add_edge dag5 vb ve;;
Dag.add_edge dag5 vc ve;;
Dag.add_edge dag5 vd vf;;
Dag.add_edge dag5 ve vf;;

Dag.dot_output dag5 "dag5.dot";;



let dag6 = Dag.create();;

let xa = Dag.createv ("a",1,2.);;
let xb = Dag.createv ("b",1,2.);;
let xc = Dag.createv ("c",1,1.);;
let xd = Dag.createv ("d",1,1.);;

Dag.add_edge dag6 xa xc;;
Dag.add_edge dag6 xb xc;;
Dag.add_edge dag6 xb xd;;

Dag.dot_output dag6 "dag6.dot";;


let afficher_vertex_liste lv =
	List.map (namev) lv;; 

let afficher_vertex_liste_liste lv =
	List.map (List.map (namev)) lv;;
...
let print_list l = 
	print_string "[" ;
		let rec aux l =
			match l with
			|[] -> print_string "\n";
			| e::l ->  print_string (namev e) ; print_string ";" ; aux l	
		in aux l;;

let print_list_list l = 
	print_string "[" ;
	let rec print_list_list_aux l = 
		match l with 
		|[]   -> print_string "\n";
		|e::l -> print_list e ; print_string ";" ; print_list_list_aux l
	in print_list_list_aux l;; 	
...
let b = 
	...
	let l = [xa] in
			let p = Dag.pred dag6 xc in
					let isInExecution = List.exists (fun e -> List.mem e l) p in
				(*let isInExecution = List.fold_right (fun curr old -> (List.mem curr p) || old) l false in *)

						(List.fold_right (fun curr old -> (Dag.computedv curr) && old) p true) && (not isInExecution);;*)



afficher_vertex_liste_liste (ordonnanceur_heterogene 20. 2 2 dag5);;
(*afficher_vertex_liste_liste (ordonnanceur_multi 3 dag5);;*)



(*let test listVertices dag  = 
	let rec test_aux listVertices returnedListe verticesEnAttente  = 
		match listVertices with 
		| []   -> 	begin
						match verticesEnAttente with 
						| []   -> (List.rev returnedListe)
						| t::q -> test_aux verticesEnAttente returnedListe []
					end
		| t::q -> 
					if (isVertexExecutable dag5 t) then
						begin
					    	Dag.decreasev t 1. ;
                        
	                        if (Dag.computedv t) then 
								test_aux q (t::returnedListe) verticesEnAttente
							else 
								(test_aux (t::q) (t::returnedListe) verticesEnAttente)
						end
					else
						test_aux q (returnedListe) (t::verticesEnAttente)

		in test_aux (getListVertex dag) [] [];;*)


(* TEST DE isVertexExecutable *)
(* let x = 2 in 
	Dag.decreasev va 1. ;
	Dag.decreasev va 1. ;
	Dag.decreasev vb 1. ;
	Dag.decreasev vb 1. ; (* A supprimer pour voir que c'est = à false*)
	
	isVertexExecutable dag5 vd;; (* = TRUE *)
*)
...
(*afficher_vertex_liste_liste (traceDisplay dag5 3);;*)

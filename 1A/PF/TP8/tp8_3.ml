...
let a = 70, [50;20;10;10],[];;
let b = 90, [50;20;50],[];;
let c = 100, [50;20;20;10],[];;
let d = 40, [10;20;20;10],[];;
...
let voisinage pos = 
        ...
        |(montant,t::q,liste) -> let nouv_montant = montant-t in 
                                if nouv_montant>=0 then 
                                    [(nouv_montant,q,liste);(montant,q,t::liste)]
                                else 
                                    [(montant,q,t::liste)];;
...
let recherche_compl (montant, pieces, unused) = 
        match List.rev (recherche voisinage critere (montant, List.sort (fun a b -> a-b) pieces , unused)) with
        |[] -> pieces
        |(_,p,q)::_ -> p@q;;

...
program mainlu
  implicit none
  interface
     subroutine init(a, acopie, xexact, b, bcopie, x, y, r, p,lda, n)
       integer :: lda,n
...
subroutine facto(m, ldm, n, p, ierr )
  ...
  !      Indices de boucle
  integer :: i,j,k, max 
  double precision :: eps, tmp(ldm), norme, det
  !
  ...
  do k = 1, n-1
        ...
        ! Recherche du max dans la colonne
        do i = k, n
           if (m(i,k) > max) then
                max = i 
           end if
        ...
        ! Erreur
        if (abs(m(max,k)) <= epsilon(eps)*norme(m,n)) then 
           exit
        end if

        ! Permutation colonne tmp et max
        tmp(1:n) = m(1:ldm, k)
        m(1:ldm, k) = m(1:ldm, max)
        m(1:ldm, max) = tmp(1:n)

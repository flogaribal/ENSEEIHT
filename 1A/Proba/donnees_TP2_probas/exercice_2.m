...
for j = 1:length(histogramme)   
    nb_bits_codage_Huffman_image_initiale = nb_bits_codage_Huffman_image_initiale+ histogramme(j)*length(dico{j,2});
end

% Calcul du coefficient de compression obtenu par codage de Huffman :
nb_bits_sans_compression = power(length(histogramme),2)*8
...
for j = 1: length(histogramme_decorrele) 
	nb_bits_codage_Huffman_image_decorrelee = nb_bits_codage_Huffman_image_decorrelee + histogramme_decorrele(j) * length(dico_decorrele{j,2})
end

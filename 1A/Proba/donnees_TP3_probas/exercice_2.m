...
for k = 1:length(liste_E)
	...
	for i = 1:card_E 
        ...
        % Calcul gradient
        gradientCourant =  gradient(E(i,:))
        Gx = gradientCourant(1)
        Gy = gradientCourant(2)
        normeGradE = sqrt(Gx^2+Gy^2)
               
         % Si gradient dans la même direction
        if abs(Gx*petit_axe(1) + Gy*petit_axe(2)) >= normeGradE * cos_alpha
            n = n+1
        end
    end
	...
	N = largeur_rectangle * hauteur_rectangle;

	% Calcul de probabilite (loi binomiale : cf. fonction binocdf) :
	y = binocdf(n, N, p);

	% Test de saillance visuelle :
	if y <= epsilon					% Si l'evenement est suffisamment rare

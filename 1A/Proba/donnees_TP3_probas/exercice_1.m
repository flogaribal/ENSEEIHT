...
cos_alpha = cos(alpha);
S = 2/sin(alpha);           		% Seuil sur la norme du gradient
...
while length(indices_tries)>0

	% Pixel dont la norme du gradient est la plus elevee = "germe" :
	indice_germe = indices_tries(1);
	[i_germe,j_germe] = ind2sub(size(I),indice_germe);
	E = [i_germe j_germe];
	G_norme_germe = G_norme(i_germe,j_germe);
	S_cos = G_x(i_germe,j_germe)/G_norme_germe;
	S_sin = G_y(i_germe,j_germe)/G_norme_germe;
	indices_tries = indices_tries(indices_tries~=indice_germe);
	contour(i_germe,j_germe) = 0;

	% Construction recursive de l'ensemble E :
	[E,S_cos,S_sin,contour,indices_tries] = recursion(E,S_cos,S_sin,contour,indices_tries,...
				i_germe,j_germe,voisins,G_x,G_y,G_norme,card_max,cos_alpha,nb_lignes,nb_colonnes);

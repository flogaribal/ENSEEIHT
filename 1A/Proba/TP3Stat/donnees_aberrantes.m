...
ecarts_angulaires_D_perp_5 = zeros(nb_essais,nb_max_donnees_aberrantes);
ecarts_angulaires_D_perp_6 = zeros(nb_essais,nb_max_donnees_aberrantes);
for i = 1:nb_essais
	...
	for j = 1:nb_max_donnees_aberrantes
        ...
        % Problème 5 
        [mini_5, indice_min_5] = min(sum(sqrt(residus_perp.^2 + alpha^2))); 		
        
        theta_perp_5 = valeurs_theta(indice_min_5);       
        ecart_angulaire_perp_5 = min(abs(theta_perp_5-theta_0),abs(theta_perp_5-theta_0+pi));
		ecart_angulaire_perp_5 = min(ecart_angulaire_perp_5,abs(theta_perp_5-theta_0-pi));
		ecarts_angulaires_D_perp_5(i,j) = ecart_angulaire_perp_5;
        
        % Problème 6 
        [mini_6, indice_min_6] = min(sum(log(residus_perp.^2 + beta^2))); 
        
        theta_perp_6 = valeurs_theta(indice_min_6);
        ecart_angulaire_perp_6 = min(abs(theta_perp_6-theta_0),abs(theta_perp_6-theta_0+pi));
		ecart_angulaire_perp_6 = min(ecart_angulaire_perp_6,abs(theta_perp_6-theta_0-pi));
		ecarts_angulaires_D_perp_6(i,j) = ecart_angulaire_perp_6;
	end
end
...
EAM_D_perp_5 = mean(ecarts_angulaires_D_perp_5)/pi*180;
EAM_D_perp_6 = mean(ecarts_angulaires_D_perp_6)/pi*180;
...
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_5,'b-.','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_1,'g-','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_6,'b-','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_2,'g-.','LineWidth',2);

lg = legend('$D_\perp$ Courbe 5','$D_\perp$ (ecarts au modele en $y$)',...
	'$D_\perp$ Courbe 6','$D_\perp$ (ecarts au modele en $x$ et en $y$)');

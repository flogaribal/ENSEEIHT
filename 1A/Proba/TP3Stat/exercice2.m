...
ecarts_angulaires_D_perp_3 = zeros(nb_essais,nb_max_donnees_aberrantes);
for i = 1:nb_essais
	...
	for j = 1:nb_max_donnees_aberrantes
		...
		residus_perp = cos_theta*x_donnees_centrees+sin_theta*y_donnees_centrees;
		arguments_perp = sum(sqrt(power(residus_perp,2) + power(alpha,2)),2);
		[minimum_perp,indice_min_perp] = min(arguments_perp);
		theta_perp = valeurs_theta(indice_min_perp);
		ecart_angulaire_perp = min(abs(theta_perp-theta_0),abs(theta_perp-theta_0+pi));
		ecart_angulaire_perp = min(ecart_angulaire_perp,abs(theta_perp-theta_0-pi));
		...
		residus_perp = cos_theta*x_donnees_centrees+sin_theta*y_donnees_centrees;
		arguments_perp = sum(log(power(residus_perp,2) + power(beta,2)),2);
		[minimum_perp,indice_min_perp] = min(arguments_perp);
		theta_perp = valeurs_theta(indice_min_perp);
		ecart_angulaire_perp = min(abs(theta_perp-theta_0),abs(theta_perp-theta_0+pi));
		ecart_angulaire_perp = min(ecart_angulaire_perp,abs(theta_perp-theta_0-pi));
...
EAM_D_perp_3 = mean(ecarts_angulaires_D_perp_3)/pi*180;
...
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_2,'b-','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,EAM_D_perp_3,'r-.','LineWidth',2);
lg = legend('$D_\perp$ (ecarts au modele en $x$ et en $y$)', '(5)', '(6)');

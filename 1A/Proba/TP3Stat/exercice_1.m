...
for i = 1:nb_essais
	...
	for j = 1:nb_max_donnees_aberrantes
		...
		for k = 1:k_max
			...
			[cos_theta,sin_theta,rho] = estimation_2_points(x_donnees(i_rand), y_donnees(i_rand));

			% Detection des donnees conformes au modele :
            r = (cos_theta * x_donnees + sin_theta * y_donnees) - rho ;
			indices_conformes = find(abs(r-seuil_distance) <= 4);
			...
			if length(indices_conformes) / nb_donnees > seuil_proportion
				...
				critere = sum(power((cos_theta * x_donnees_conformes + sin_theta * y_donnees_conformes) - rho,2),2) / length(indices_conformes) ;

...
p = 0.10;
n = 200;
nb_points_final = round(n*(1-p));
sigma = 0.5;
rho_donnees = R_0+sigma*randn(1,nb_points_final);
theta_donnees = 2*pi*rand(1,nb_points_final);
xy_donnees = C*ones(1,nb_points_final)+[rho_donnees.*cos(theta_donnees) ; rho_donnees.*sin(theta_donnees)];

% Donnees aberrantes
aberr = taille*(2*rand(2,round(n*p))-1);
xy_donnees = [xy_donnees aberr]

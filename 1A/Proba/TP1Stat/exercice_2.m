...
rayon_test = taille*(2*rand(1,nb_tirages)-1); 
arguments = [];
for k = 1:nb_tirages
    arguments(k) = sum(((sqrt((xy_donnees(1,:)-C_test(1,k)).^2 + (xy_donnees(2,:)-C_test(2,k)).^2))-rayon_test(k)).^2);
end
[argument_min,indice_min] = min(arguments);

C_estime = [C_test(1,indice_min);C_test(2,indice_min)]
rayon_test(indice_min)

xy_cercle = [rayon_test(indice_min)*cos(theta_cercle) ; rayon_test(indice_min)*sin(theta_cercle)]+C_estime*ones(1,nb_points_cercle);

...
int appartient(int pid, ListeProc liste_proc){
  ...
  /* Fin de parcours de liste ou processus recherché trouvé */
...
void afficher_liste_proc(ListeProc liste_proc){
    printf("\n");
    if (liste_proc == NULL) {
	     printf("La liste ne comporte aucun processus\n");
    }else{ 
	    while(liste_proc != NULL) {
    		printf("PID = %d \n", liste_proc -> pid);
    		printf("COMMANDE = %s \n", liste_proc -> nomCommande);
    		/*printf("USER = %s \n", liste_proc -> nomUser);*/
    		/*printf("MACHINE SOURCE = %s \n", liste_proc -> nomMachineSource);*/

    		/* Affichage état du processus */
    		if (liste_proc -> etat == 0){
...
/* fonction inserer_proc_en_queue : 
  ...
  -- paramètres : 
	...
	char *nomCmd : Nom de la commande exécutée par le processus (D)
  ...
  -- post : aucune 
  --------------------------------------------------*/
int inserer_proc_en_queue(int pid, State etat, char *nomCmd, ListeProc *liste_proc){
...
int modifier_etat_proc(int pid, State etat, ListeProc liste_proc){
    ...
    /* Fin de parcours de list eou processus recherché trouvé */
...
#include <unistd.h>
    if(tmp_liste -> pid == pid){
        *liste_proc = tmp_liste -> suivant;
        return 0;
    }
...
/* fonction lire_etat_proc : 
  -- lit l'état du processus de n° pid
  -- paramètres : 
        int pid : n° du processus (D)
        ListeProc liste_proc : liste de processus (D) 
                 [attention : la structure de liste n'est pas modifiée]
  -- résultat :
        int : 0 SUSPENDU, 1 ACTIF, -1 processus non trouvé
  -- pré : aucune
  -- post : aucune 
  --------------------------------------------------*/
int lire_etat_proc(int pid, ListeProc liste_proc){
    ...
    if(liste_proc == NULL){
        ...
        return liste_proc -> etat;
    }
}

/* fonction détruire_liste : 
  -- détruit la liste de processus
  -- paramètres : 
        ListeProc liste_proc : liste de processus (R) 
                  [attention : la structure de liste est modifiée]
  -- pré : aucune
  -- post : aucune 
  --------------------------------------------------*/
void detruire_liste(ListeProc *liste_proc){
    ListeProc tmp_liste, suiv;
    int pid;
    char pidToKill [4];
    ...
    if(liste_proc == NULL){
    	...
    	tmp_liste = *liste_proc;
    	while(tmp_liste != NULL) {
    		suiv = tmp_liste -> suivant;
    		pid = fork(); 
    		...
    		if(pid == -1){		/* ERREUR */
    		    ...
    		    sprintf(pidToKill,"%d", tmp_liste -> pid);

    		    execlp("kill","kill","-9", pidToKill, NULL);
